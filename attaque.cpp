...
int attacked(TChessBoard *cb, int i_iCase, int i_bWtm)
{
  ...
  if ( i_bWtm ) {
    ...
    // Les fous.
    attaque = attaqueFou(cb, i_iCase) & cb->foub;
    if ( attaque ) return fou;
    // Les tours.
    attaque = attaqueTour(cb, i_iCase) & cb->tourb;
    if ( attaque ) return tour;
    // Les dames.
    attaque = attaqueDame(cb, i_iCase) & cb->dameb;
    ...
    if ( attaque ) return cavalier;
    // Les fous.
    attaque = attaqueFou(cb, i_iCase) & cb->foun;
    if ( attaque ) return fou;
    // Les tours.
    attaque = attaqueTour(cb, i_iCase) & cb->tourn;
    if ( attaque ) return tour;
    // Les dames.
    attaque = attaqueDame(cb, i_iCase) & cb->damen;
    if ( attaque ) return dame;
    // Le roi.
...
Bitboard attaqueDe(TChessBoard *cb, int i_iCase, int wtm)
{
  ...
  if ( wtm ) {
    ...
    map |= attaqueTour(cb, i_iCase) & cb->tourb;
    map |= cb->AttaquePionNoir[i_iCase] & cb->pionb;
    map |= attaqueFou(cb, i_iCase) & cb->foub;
    map |= attaqueDame(cb, i_iCase) & cb->dameb;
    map |= cb->AttaqueRoi[i_iCase] & cb->roib;
    return map;
  }
  else {
    map = cb->AttaqueCavalier[i_iCase] & cb->cavaliern;
    map |= attaqueTour(cb, i_iCase) & cb->tourn;
    map |= cb->AttaquePionBlanc[i_iCase] & cb->pionn;
    map |= attaqueFou(cb, i_iCase) & cb->foun;
    map |= attaqueDame(cb, i_iCase) & cb->damen;
